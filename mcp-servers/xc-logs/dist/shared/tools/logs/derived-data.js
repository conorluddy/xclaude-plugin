 ...
 */
export async function findLatestXcresult(projectPath) {
    ...
    try {
        ...
        if (projectPath) {
            ...
            const matching = projectDirs.filter((d) => d.name.startsWith(projectName + "-"));
        ...
        logger.debug(`Found ${xcresults.length} xcresult bundles, using most recent: ${xcresults[0].path}`);
