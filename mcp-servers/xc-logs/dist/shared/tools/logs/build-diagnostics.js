...
export const logsBuildDiagnosticsDefinition = {
    name: "logs_build_diagnostics",
    description: "Parse structured build errors and warnings from an xcresult bundle. Auto-discovers the latest xcresult if no path is provided.",
    inputSchema: {
        type: "object",
        properties: {
            xcresult_path: {
...
export async function logsBuildDiagnostics(params) {
    try {
        // Find xcresult
        ...
        if (!xcresultPath) {
            return {
                success: false,
                error: "No .xcresult bundle found. Build the project first or provide an explicit xcresult_path.",
        ...
        const result = await runCommand("xcrun", ["xcresulttool", "get", "build-results", "--path", xcresultPath], { timeout: LOGS_CONFIG.XCRESULTTOOL_TIMEOUT_MS });
        if (result.code !== 0) {
            return {
                success: false,
        ...
        logger.error("Failed to parse build diagnostics", error);
        return {
            success: false,
            error: String(error),
            operation: "logs_build_diagnostics",
