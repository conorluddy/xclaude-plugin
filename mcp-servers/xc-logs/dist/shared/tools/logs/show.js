...
export const logsShowDefinition = {
    name: "logs_show",
    description: "Read recent app console output from macOS unified logging. Provide at least process or subsystem to filter logs.",
    inputSchema: {
        type: "object",
        properties: {
            process: {
...
export async function logsShow(params) {
    try {
        ...
        const lastMinutes = Math.min(params.last_minutes ?? LOGS_CONFIG.DEFAULT_LAST_MINUTES, LOGS_CONFIG.MAX_LAST_MINUTES);
        const maxEntries = Math.min(params.max_entries ?? LOGS_CONFIG.DEFAULT_MAX_ENTRIES, LOGS_CONFIG.MAX_ENTRIES_CEILING);
        ...
        logger.info(`Reading logs: predicate='${predicate}', last=${lastMinutes}m, level=${level}`);
        ...
        for (const line of lines) {
            try {
                ...
                entries.push({
                    ...
                    process: parsed.processImagePath?.split("/").pop() || parsed.process || "",
