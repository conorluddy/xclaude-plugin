...
export type { TestResultsParams, TestResultsResultData };

export const logsTestResultsDefinition: ToolDefinition = {
  name: "logs_test_results",
  description:
    "Parse test pass/fail results from an xcresult bundle. Shows counts and details for failed tests. Auto-discovers the latest xcresult if no path is provided.",
  inputSchema: {
    type: "object",
    properties: {
      xcresult_path: {
...
export async function logsTestResults(
  params: TestResultsParams,
): Promise<ToolResult<TestResultsResultData>> {
  try {
    // Find xcresult
    const xcresultPath =
    ...
    if (!xcresultPath) {
      return {
        success: false,
        error:
          "No .xcresult bundle found. Run tests first or provide an explicit xcresult_path.",
    ...
    const failedTests: TestCaseResult[] = [];
    ...
    if (testsResult.code === 0 && testsResult.stdout.trim()) {
      try {
        ...
        logger.debug(
          "Failed to parse test tree JSON, skipping failure details",
        );
      }
    }

    const data: TestResultsResultData = {
    ...
    logger.error("Failed to parse test results", error as Error);
    return {
      success: false,
      error: String(error),
      operation: "logs_test_results",
