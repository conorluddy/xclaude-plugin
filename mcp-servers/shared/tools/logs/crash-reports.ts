...
export type { CrashReportsParams, CrashReportsResultData };

export const logsCrashReportsDefinition: ToolDefinition = {
  name: "logs_crash_reports",
  description:
    "Read and parse crash reports (.ips files) with backtraces. Searches macOS DiagnosticReports and optionally simulator crash directories.",
  inputSchema: {
    type: "object",
    properties: {
      app_name: {
        type: "string",
        description:
          "Filter by app/process name (matches filename and process name)",
      },
      max_reports: {
        type: "number",
      ...
      include_simulator: {
        type: "boolean",
        description:
          "Also search CoreSimulator crash directories (default: false)",
      },
    },
  },
};

export async function logsCrashReports(
  params: CrashReportsParams,
): Promise<ToolResult<CrashReportsResultData>> {
  try {
    const maxReports = Math.min(
      params.max_reports ?? LOGS_CONFIG.DEFAULT_MAX_CRASH_REPORTS,
      LOGS_CONFIG.MAX_CRASH_REPORTS_CEILING,
    );
    ...
    const ipsFiles: Array<{ path: string; mtime: number }> = [];

    for (const dir of searchDirs) {
      try {
        const files = await readdir(dir, { withFileTypes: true });
        for (const file of files) {
          if (!file.name.endsWith(".ips")) continue;
          ...
          try {
            ...
            if (stats.mtimeMs >= cutoffTime) {
              if (stats.size > LOGS_CONFIG.MAX_IPS_FILE_SIZE_BYTES) {
                logger.debug(
                  `Skipping oversized crash report: ${fullPath} (${stats.size} bytes)`,
                );
                continue;
              }
    ...
    const data: CrashReportsResultData = {
      reports,
    ...
    logger.error("Failed to read crash reports", error as Error);
    return {
      success: false,
      error: String(error),
 ...
 */
function parseIpsFile(content: string, filePath: string): CrashReport | null {
  try {
    ...
    // Extract crash info
    const processName =
      data.procName || data.name || data.processName || "unknown";
    const timestamp = data.captureTime || data.timestamp || data.date || "";

    // Exception info
    const exceptionType =
      data.exception?.type || data.termination?.signal || undefined;
    ...
    if (threads.length > faultingThread) {
      ...
      for (
        let i = 0;
        i < Math.min(frames.length, LOGS_CONFIG.MAX_BACKTRACE_FRAMES);
        i++
      ) {
