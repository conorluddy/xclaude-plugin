...
import type { DescribeParams, DescribeResultData, UIElement } from '../../types/idb.js';
...
export async function idbDescribe(params: DescribeParams): Promise<ToolResult<DescribeResultData>> {
  try {
    ...
    logger.error('Describe failed', error as Error);
    return {
      success: false,
      error: String(error),
      operation: 'describe',
